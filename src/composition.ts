 ...
 * Composition root
 *
 * The single place that assembles the Conductor runtime (storage →
 * repositories → services → manager). CLI, UI server, mounted DSH plugin
 * and demos all compose through here, so there is exactly one definition
 * of "what the control plane contains".
 */
...
export function resolveDbPath(): string {
  ...
  return resolve(process.cwd(), '.conductor', 'conductor.db');
}
